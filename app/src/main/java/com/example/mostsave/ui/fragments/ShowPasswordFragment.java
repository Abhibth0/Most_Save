...
public class ShowPasswordFragment extends Fragment {
    ...
    private void sharePassword() {
        if (currentPassword != null) {
            String shareText = "This Password Is Shared By MostSave\n\n" +
                    "Id: " + (currentPassword.getUsername() != null ? currentPassword.getUsername() : "N/A") + "\n" +
                    "Password: " + currentPassword.getPassword() + "\n\n" +
                    "⚠️ Do not forward this message to unknown persons";

            Intent shareIntent = new Intent(Intent.ACTION_SEND);
            shareIntent.setType("text/plain");
            shareIntent.putExtra(Intent.EXTRA_TEXT, shareText);
            startActivity(Intent.createChooser(shareIntent, "Share password via"));
        } else {
            Snackbar.make(requireView(), "No password details to share", Snackbar.LENGTH_SHORT).show();
        }
    }

    private void showMoreOptionsMenu() {
        // A robust way to find the anchor view for the menu item
        View anchorView = requireActivity().findViewById(R.id.action_more);
        if (anchorView == null) {
            // Last resort
            anchorView = requireView();
        }

        // Create PopupMenu with the correct anchor
        PopupMenu popupMenu = new PopupMenu(requireContext(), anchorView, android.view.Gravity.END);
